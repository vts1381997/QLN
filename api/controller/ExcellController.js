...
const ExportTamplate = {
    ...
    tochuctaichinh: (data, callback) => {
        ...
        var keys = []
        try {
            Query(data, 'fn_dm_tochuctaichinh_gettemplate', function (_rs) {
                _rs_format = []
                keys = _rs.length > 0 ? Object.keys(_rs[0]) : []
                _rs = _rs.slice(0, 1)
                _rs.map((item, index_item) => {
                ...
                XlsxPopulate.fromBlankAsync()
                    .then(workbook => {
                        ...
                        _rs_format.map((item_rs, index_rs) => { // vẽ body
                            arr_cell.map((itemCell, index_cell) => {
                        ...
                        var filename = 'file_mẫu_nhập_dữ_liệu_tổ_chức_tài_chính_tín_dụng' + (((new Date()).getUTCDay() + 7) + '-' + (new Date()).getMonth() + '-' + (new Date()).getFullYear() + '-') + ((new Date()).getTime()) + idlogin + '.xlsx'
                        var urlFile = 'storage/excell/tamplate/' + filename
...
const Import = {
    ...
    tochuctaichinh: (data, callback) => {
        var id_user = data.idlogin
        try {
            delete data.i_sheetimport['Danh sách TCTCTD']['!merges']
            delete data.i_sheetimport['Danh sách TCTCTD']['A1']
            var arr_data_tctctd = Xlsx.utils.sheet_to_json(data.i_sheetimport['Danh sách TCTCTD'])
            var arr_data_tctctd_format = [] 
            var keys_tctctd = Object.keys(arr_data_tctctd[0]) 
            arr_data_tctctd.map((_item_hopdongvaylai, index_hdvl) => {
                if (index_hdvl > 1) {
                    arr_data_tctctd_format.push({ MA: _item_hopdongvaylai[keys_tctctd[1]], TEN: _item_hopdongvaylai[keys_tctctd[2]], DIACHI: _item_hopdongvaylai[keys_tctctd[3]], SOGIAYPHEP: _item_hopdongvaylai[keys_tctctd[4]], NGAYCAP: _item_hopdongvaylai[keys_tctctd[5]], VONDIEULE: _item_hopdongvaylai[keys_tctctd[6]], TRANGTHAI: _item_hopdongvaylai[keys_tctctd[7]], STATUS: 1, CREATEDBY: id_user, UUID: uuidv4() })
                }
            })

            str_insert_head = 'insert into dm_tochuctaichinh ( TOCHUCTAICHINHID, MA, TEN, DIACHI, SOGIAYPHEP, NGAYCAP,VONDIEULE, TRANGTHAI, STATUS ,CREATEDBY, UUID ) values '
            str_insert_value = ''
            arr_data_tctctd_format.map((value, index) => {
                str_insert_value += " ( dm_tochuctaichinh_seq.nextval + " + index + ", '" + value.MA + "', '" + value.TEN + "', '" + value.DIACHI + "', " + value.SOGIAYPHEP + ", to_date('" + value.NGAYCAP + "','DD/MM/YYYY'), " + value.VONDIEULE + ', ' + value.TRANGTHAI + ', ' + value.STATUS + ", '" + value.CREATEDBY + "', '" + value.UUID + "' ), "
            }) 
             
            var str_sql = str_insert_head + str_insert_value.slice(0, -2);
            // var data = {};
            // data.i_delete = str_sql;
            // data.i_giatri = '';
            // Query(data, 'fn_qln_kehoachvayhangnam_dtl_save', (_rs_hopdong) => {
            //     console.log(_rs_hopdong);
            //     callback({ CODE: 1, MESSAGE: 'Nhận dữ liệu thành công' })
            // }).catch(err => {
            //     console.log(err);
            // }) 
            Knex.raw(str_sql).then(_rs_select => {
                callback({ CODE: 1, MESSAGE: 'Nhận dữ liệu thành công' })
            }).catch(err => {
                console.log(err);
            })
