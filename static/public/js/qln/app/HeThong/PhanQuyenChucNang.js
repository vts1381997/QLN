var PhanQuyenChucNang = function(){
	
	var that = this;
	this.AppTitle = 'Phân quyền chức năng';
	this.Id='';
	this.oTable = null;
	this.oDialog = null;
	...
	$(function() {
		...
		$('.ACTIONS').on('click', '#btnAddNew', function () {
			that.oDialog.show('Thêm Phân quyền chức năng', 'PhanQuyenChucNangCT', '80%', '500px');
		});		
		$('.ACTIONS').on('click', '#btnDelete', function () {
			if (!that.oDmKhachHang.KHACHHANGID) {
				var oAlert = new AlertDialog('Cảnh báo');
				oAlert.show('Chưa chọn Phân quyền chức năng để xóa','40%', '50px');
				return false;
			}
