var VayQuyDuTruTinh = function(){
	...
	var that = this;
	this.AppTitle = 'Vay quỹ dự trữ tỉnh';
	this.Id='';
	this.oTable = null;
	this.oDialog = null;
	...
	$(function() {
		...
		$('.ACTIONS').on('click', '#btnAddNew', function () {
			that.oDialog.show('Thêm thông tin vay quỹ dự trữ tỉnh', 'VayQuyDuTruTinhCT', '80%', '500px');
		});		
		$('.ACTIONS').on('click', '#btnDelete', function () {
			if (!that.oDmKhachHang.KHACHHANGID) {
				var oAlert = new AlertDialog('Cảnh báo');
				oAlert.show('Chưa chọn thông tin vay quỹ dự trữ tỉnh để xóa','40%', '50px');
				return false;
			}
