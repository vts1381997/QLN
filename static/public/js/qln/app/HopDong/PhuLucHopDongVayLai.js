var PhuLucHopDongVayLai = function(){
	...
	var that = this;
	this.AppTitle = 'File hợp đồng và phụ lục hợp đồng vay lại';
	this.Id='';
	this.oTable = null;
	this.oDialog = null;
	...
	$(function() {
		...
		$('.ACTIONS').on('click', '#btnAddNew', function () {
			that.oDialog.show('Thêm file hợp đồng và phụ lục cho vay lại', 'PhuLucHopDongVayLaiCT', '80%', '500px');
		});		
		$('.ACTIONS').on('click', '#btnDelete', function () {
			if (!that.oDmKhachHang.KHACHHANGID) {
				var oAlert = new AlertDialog('Cảnh báo');
				oAlert.show('Chưa chọn file hợp đồng và phụ lục để xóa','40%', '50px');
				return false;
			}
