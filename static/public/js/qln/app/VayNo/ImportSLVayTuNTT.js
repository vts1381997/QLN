var ImportSLVayTuNTT = function(){
	
	var that = this;
	this.AppTitle = 'Nhập số liệu vay từ nhà tài trợ';
	this.Id='';
	this.oTable = null;
	this.oDialog = null;
	...
	$(function() {
		...
		function reload(){
			...
			e.preventDefault()                                                                               
			that.oDialog.show('Thêm Nhập số liệu vay từ nhà tài trợ', 'ImportSLVayTuNTTCT', '80%', '500px');
		});		
		$('.ACTIONS').on('click', '#btnDelete', function (e) {
				e.preventDefault()                                                        
			if (!that.oDmKhachHang.KHACHHANGID) {
				var oAlert = new AlertDialog('Cảnh báo');
				oAlert.show('Chưa chọn Nhập số liệu vay từ nhà tài trợ để xóa','40%', '50px');
				return false;
			}
